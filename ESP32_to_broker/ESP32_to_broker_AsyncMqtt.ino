/*
  pubSubClient vs asyncmqttclient
  (not an exact answer)- A synchronous implementation runs in the main loop. So for example, to connect to a TCP socket, if it takes 3 seconds, the whole code would block for 3 seconds. An asynchronous implementation does not run in the main loop. When you will connect to the TCP socket, it will work in the background and call your code back (callback) when it's done. So it's not blocking, you can do whatever you want in between! Simply put, the advantage here it is faster and more bulletproof. You can easily crash PubSubClient under high load, nobody managed to break AsyncMqttClient yet.
*/
...
#define WIFI_PASSWORD "JDJ57TE655L"
...
// MQTT Topics
// const char* MQTT_PUB_MOIS "FIELD/ESP32/YL69";
#define MQTT_PUB_MOIS "FIELD/ESP32/YL69"
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    // Publish an MQTT message on topic "FIELD/ESP32/YL69"
